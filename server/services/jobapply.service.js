...
const methods = {
    async findAll(req) {
        try {
            ...
            const rows = await JobApply.find()
                .sort({ createdAt: "desc" })
                .limit(limit)
                .skip(offset);
            const count = await JobApply.countDocuments();
            return {
                total: count,
    ...
    async findById(id) {
        try {
            const obj = await JobApply.findById(id);
    ...
    async update(req, res) {
        return new Promise((resolve, reject) => {
            createUploader("./public/file/").fields([
                ...
                try {
                    const data = req.body;
                    const obj = await JobApply.findById(req.params.id);
                    ...
                    await JobApply.updateOne({ _id: req.params.id }, data, {
                        runValidators: true,
                        new: true,
                    });
    ...
    async delete(id) {
        try {
            const obj = await JobApply.findOneAndDelete({ _id: id }).exec();
            if (!obj) return Promise.reject(ErrorNotFound("id: not found"));
